...
class Commands:
    ...
    # Sent by the device without requiring either an MQTT or application ACK.
    DEVICE_LOG_REPORT_EVENT: str = 'DEVICE_LOG_REPORT_EVENT'
...
@dataclass
class DeviceLogReportEventIn:
    message_id: MessageId
    timestamp: Timestamp
    logs: list[dict]

    @staticmethod
    def from_mqtt_payload(payload: dict) -> DeviceLogReportEventIn:
        return DeviceLogReportEventIn(
            message_id = MessageId(payload['msgId']),
            timestamp = Timestamp.from_timestamp_epoch_ms(int(payload['ts'])),
...
class Client:
    def __init__(self, ad: adapi.ADAPI, mqtt: mqttapi.Mqtt, device_serial_number: str):
        ...
        self.device_config_sync_callback = None
    ...
    def device_config_sync_listen(self, callback):
        self.device_config_sync_callback = callback
    ...
    def device_config_sync_send(self, device_config_sync_out: DeviceConfigSyncOut):
        self._mqtt_service_send(device_config_sync_out.to_mqtt_payload())
    ...
    def get_feeding_plan_event_send(self, get_feeding_plan_event_out: GetFeedingPlanEventOut):
        self._mqtt_event_send(get_feeding_plan_event_out.to_mqtt_payload())

    def grain_output_event_send(self, grain_output_event_out: GrainOutputEventOut):
        self._mqtt_event_send(grain_output_event_out.to_mqtt_payload())
    ...
    def server_config_push_send(self, server_config_push_out: ServerConfigPushOut):
        self._mqtt_config_send(server_config_push_out.to_mqtt_payload())
    ...
    def _mqtt_recv_service_cb(self, eventname: str, data: dict, kwargs):
        ...
        if cmd == Commands.ATTR_SET_SERVICE:
            ...
            if not self.feeding_plan_service_callback == None:
                feeding_plan_service_in = FeedingPlanServiceIn.from_mqtt_payload(payload)
                self.feeding_plan_service_callback(feeding_plan_service_in)
        elif cmd == Commands.DEVICE_CONFIG_SYNC:
            if not self.device_config_sync_callback == None:
                device_config_sync_in = DeviceConfigSyncIn.from_mqtt_payload(payload)
                self.device_config_sync_callback(device_config_sync_in)
